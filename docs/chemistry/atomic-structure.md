...
| 铁 (Fe) | $\mathrm{1s^2 2s^2 2p^6 3s^2 3p^6 4s^2 3d^6}$ |
...
| 铁 (Fe) | $\mathrm{[Ar] 3d^6 4s^2}$ | $\mathrm{1s^2 2s^2 2p^6 3s^2 3p^6 4s^2 3d^6}$ |
...
因为（[性质 6](#_4)），电子不一定优先填充低能层高能级的原子轨道，反而会填充高能层低能级的原子轨道，这样可以使总体能量更低。
...
这会带来一些不寻常的电子排布方式，例如：铜（$\mathrm{4s^1 3d^{10}}$），铬（$\mathrm{4s^1 3d^5}$）。

以铬为例，若不考虑洪特规则的特例，其电子排布按正常填充顺序本应为 $\mathrm{4s^2 3d^4}$。但实验测定其基态排布为 $\mathrm{4s^1 3d^5}$——此时 4s 轨道半满、3d 轨道半满，整个体系能量更低、更稳定。铜的 $\mathrm{4s^1 3d^{10}}$ 排布（3d 全满、4s 半满）也是同理。
